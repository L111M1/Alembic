...
| `params` | dict | 否 | 传给 LLM 的参数字典，支持任意 OpenAI 兼容参数<br>`temperature`, `max_tokens`, `top_p`, `frequency_penalty`, `presence_penalty`, `stop` 等 |
...
3. 相似度 ≥ `embedding_similarity_threshold`（默认 0.85）则视为重复，保留第一条
...
| `embedding_similarity_threshold` | float | 语义去重阈值 |
